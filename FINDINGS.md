...
| F-22 | the collector-artifact fetch URL (library CP-34) | FRICTION | the three READMEs fetched `collector-requirements.txt` at `…/v0.7.0/devharness/uniagent/…` — dead on BOTH halves: no `v0.7.0` tag was ever cut (the library spent CP-33 on its corpus pipeline), and the library's M8b purge (ADR-0048) moved the artifact to `staging/collector/` | a raw-at-**tag** URL is only as stable as the promise that the tag gets cut; nothing here fails loudly until someone builds a venv from scratch | repointed to a **commit-sha** raw URL at the library's CP-34 commit `baac10b` — immutable by construction, and it names the post-purge path | library-side: the library's CP-35 publishes `v0.8.0`; this repo returns to the tag form `…/v0.8.0/staging/collector/…` then | **closed (library v0.8.0, CP-35)** — the library published v0.8.0 (the first release carrying the artifact) and all three READMEs + `requirements.txt` are back on tag URLs: `…/v0.8.0/staging/collector/collector-requirements.txt` and the 0.8.0 wheel release URL. Verified 200 with matching sha256 `9a79ad35…` |
